...
definition(
    name: "Abnormal Power Consumption",
    namespace: "smartthings",
    author: "Tats De",
    description: "Alerts the user if Power Consumption is abnormal",
...
preferences() {
	...
	section("Power threshold when system is active..." ) {
		input "activeKW", "decimal", title: "kW when system is active..."
	}
...
private evaluate()
{
	if (sensor) {
        ...
        if (tm in ["cool","auto"]) {
        	...
        	if (currentTemp - coolingSetpoint >= threshold && (!(thermostat.thermostatOperatingState in ["cooling"]))) {
        	...
        	else if( (coolingSetpoint - currentTemp >= threshold) && (thermostat.thermostatOperatingState in ["cooling"])){
